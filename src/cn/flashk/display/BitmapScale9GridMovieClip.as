package cn.flashk.display
{
	...
	/**
	 * 支持位图九宫格缩放的MovieClip，如果有需要在MovieClip中使用位图九宫格，请直接从此类继承，MovieClip本身的set width和set height函数已经被重写，然后直接使用s.width=150这样的操作就可以了（如果是使用scaleX属性设置，请在设置完后调用updta方法）
	 */ 
	public class BitmapScale9GridMovieClip extends MovieClip
	{
		/**
		 * 标记哪些子显示对象需要显示原生对象。在此数组中的显示对象将设visible = true同时不会被绘制到bitmap表面，其他此MovieClip中的所有子显示对象将visible = false并都绘制到bitmap中。
		 * 也就是此数组中的显示对象采用flash自己的缩放，而其他子显示对象采用九宫格缩放。如果有需要用户选择的文本等请放到此数组中，因为如果不这样，文本将会变成bitmap位图。
		 * 九宫格操作是针对绘制后的bitmap的，默认visibleChildrens为空数组(请勿将此属性设为null，改用[]），也就是绘制九宫格缩放后所有此MovieClip子显示对象将隐藏（除了DisplayObjectDrawScale9GridBitmap本身）
		 */ 
		...
		public function BitmapScale9GridMovieClip()
		{
			super();
		...
		/**
		 * 立即刷新MovieClip的九宫格显示，如果一个MovieClip被放置到了舞台而且使用了Flash CS的缩放工具缩放，请在构造函数中调用此方法（或者在其他代码中调用此方法）
		 */ 
